...
export default function App() {
  ...
  // Handler: Update Context Profile
  const handleSaveContext = (updated: ContextProfile) => {
    setContext(updated);
  };

  // Handler: Add Universal Control
  const handleAddControl = (newCtrl: UniversalControl) => {
    setControls((prev) => [newCtrl, ...prev]);
  };

  // Handler: Sync Connector
  ...
  // Handler: Add Evidence Artifact
  const handleAddEvidence = (newArt: EvidenceArtifact) => {
    setEvidence((prev) => [newArt, ...prev]);
  };

  // Handler: Add / Approve / Reject Agent Runs
  const handleAddAgentRun = (run: AgentRunRecord) => {
    setAgentRuns((prev) => [run, ...prev]);
  };

  const handleApproveRun = (runId: string) => {
    setAgentRuns((prev) =>
      prev.map((r) => (r.id === runId ? { ...r, status: "Approved" } : r))
    );
  };

  const handleRejectRun = (runId: string) => {
    setAgentRuns((prev) =>
      prev.map((r) => (r.id === runId ? { ...r, status: "Rejected" } : r))
    );
  };

  // Handler: Add Risk & Vendor
  ...
  // Handler: Run Automated Test
  const handleTriggerTest = (testId: string) => {
    setTests((prev) =>
      prev.map((t) =>
        t.id === testId
          ? { ...t, lastRun: "Just now", result: "PASS" }
          : t
      )
    );
  };

  // Pending reviews count for Agents badge
      ...
      <div className="flex flex-1 w-full overflow-hidden">
        ...
        <main className="flex-1 flex flex-col h-full overflow-y-auto bg-white">
          ...
          <footer className="h-8 bg-slate-50 border-t border-slate-100 px-8 flex items-center justify-between text-[9px] font-mono text-slate-400 uppercase tracking-widest shrink-0">
            <span>Normora Orchestrator v1.0.4-stable • Continuous Controls Engine</span>
            <span>© 2026 Normora AI Assurance • Zero-Trust Compliance</span>
          </footer>
        </main>
      </div>
    </div>
